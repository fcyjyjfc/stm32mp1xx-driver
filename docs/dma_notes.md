...
### DMAMUX 寄存器 (pp.1233-1236, 19.6)

> DMAMUX 寄存器可 8/16/32 位访问，地址须对齐。

#### DMAMUX_CxCR — 多路复用器通道配置 (x=0~15)

偏移：`0x000 + 0x04 * x`，复位值 0x00000000

| 位 | 名称 | 读写 | 说明 |
|----|------|------|------|
| 26:24 | SYNC_ID[2:0] | rw | 同步输入选择 (Table 113) |
| 23:19 | NBREQ[4:0] | rw | 同步后放行 NBREQ+1 个请求，**SE=0 且 EGE=0 才可写** |
| 18:17 | SPOL[1:0] | rw | 00=无 01=上升沿 10=下降沿 11=双边沿 |
| 16 | SE | rw | 同步使能 |
| 9 | EGE | rw | 事件发生使能 |
| 8 | SOIE | rw | 同步溢出中断使能 |
| 6:0 | DMAREQ_ID[6:0] | rw | 请求输入选择 (Table 111)，0=无请求 |
...
| 位 | 名称 | 读写 | 说明 |
|----|------|------|------|
| 15:0 | SOF[15:0] | r | 各通道同步溢出标志，写 1 到 CFR.CSOFx 清除 |
...
| 位 | 名称 | 读写 | 说明 |
|----|------|------|------|
| 15:0 | CSOF[15:0] | w | 写 1 清除对应 SOFx |

#### DMAMUX_RGxCR — 请求发生器通道配置 (x=0~7)

偏移：`0x100 + 0x04 * x`，复位值 0x00000000

| 位 | 名称 | 读写 | 说明 |
|----|------|------|------|
| 23:19 | GNBREQ[4:0] | rw | 每次触发产生 GNBREQ+1 个请求，**GE=0 才可写** |
| 18:17 | GPOL[1:0] | rw | 00=无 01=上升沿 10=下降沿 11=双边沿 |
| 16 | GE | rw | 发生器使能 |
| 8 | OIE | rw | 触发溢出中断使能 |
| 2:0 | SIG_ID[2:0] | rw | 触发输入选择 (Table 112) |
...
| 位 | 名称 | 读写 | 说明 |
|----|------|------|------|
| 7:0 | OF[7:0] | r | 各发生器通道触发溢出标志，写 1 到 RGCFR.COFx 清除 |
...
| 位 | 名称 | 读写 | 说明 |
|----|------|------|------|
| 7:0 | COF[7:0] | w | 写 1 清除对应 OFx |

#### 寄存器偏移汇总

| 偏移 | 寄存器 | 数量 |
|------|--------|------|
| 0x000~0x03C | DMAMUX_CxCR | 16 个通道 |
| 0x080 | DMAMUX_CSR | 1 |
| 0x084 | DMAMUX_CFR | 1 |
| 0x100~0x11C | DMAMUX_RGxCR | 8 个通道 |
| 0x140 | DMAMUX_RGSR | 1 |
| 0x144 | DMAMUX_RGCFR | 1 |
